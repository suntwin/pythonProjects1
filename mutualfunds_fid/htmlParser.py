...
class  HtmlParser:
	...
	def __init__(self,htmlurl,fileloc):
		...
		try:
			...
			raise MutualFunds_URL_Error("Website cannot be reached",'101')

		except IOError as e:
			raise MutualFunds_IO_Error("This is a IO Error",'102')
	
		except urllib2.HTTPError as e:
			raise MutualFunds_URL_Error("This is an http Error",'103')
	...
	def extract_link_table_tag(self):
		...
		table = None
		print("tables_resultset",tables_resultset)
	...
	def extract_table_by_class(self,class_name):
			...
			try:
				...
				raise MutualFunds_TableByClass_Extraction_Error("Error while extracting table based on class",'104')
			return(data)
	...
	def extract_hyper_link_info(self,links,field_names):
		link_list = []
		try:
			for link in links:
				if(link.contents != []):
				...
				raise MutualFunds_HyperLink_Extraction_Error("Error while extracting Hyper Link information",'105')
		return(link_list)
