...
"""initialise the CSV writer to start writing the holdings in a separate csv file"""
...
for row in data_list:
	if(row['Do_Not_Use']!='X'):
		...
		logger.debug("Processing Url for fund %s %s %s",row['code'],row['fundname'],portfolio_holdings_url)
		...
		if (ds_utility != None):
			try:
				...
				table_with_codes = ds_utility.add_fund_and_code(table,row['fundname'],row['code'],portfolio_fieldnames.split(','))
